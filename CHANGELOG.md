...
- Security: the FTS5 `MATCH` builder (`buildMatchQuery`) neutralises FTS5 metacharacters by wrapping the user query as a single literal phrase (double-quote-wrap, double internal quotes), so no FTS5 operator from user text (`AND`/`OR`/`NOT`/`NEAR`/`*`/`"`/`:`/`^`) reaches the parser — `?`-binding alone does NOT close this second-order injection. An adversarial unit test covers the exact payload set. `docs/adrs/ADR-013-bm25-fts5-keyword-recall.md` records the build-tag + RRF-recall-fusion decisions; `docs/evals/bm25-results.md` records the same-session before/after eval (both rerank paths, no regression) and the tokenizer choice. [agentic-2lak]
