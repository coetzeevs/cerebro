...
func TestFreshInitHasValidityColumnsAtV4(t *testing.T) {
	...
	if ver != "7" {
		t.Fatalf("expected schema_version=7 on fresh Init, got %q", ver)
	}
...
func TestMigrationFromV3AddsValidityColumns(t *testing.T) {
	...
	if ver != "7" {
		t.Fatalf("expected schema_version=7 after v3->v4->v5 ladder, got %q", ver)
	}
...
func TestMigrationFromV3Idempotent(t *testing.T) {
	...
	if ver != "7" {
		t.Fatalf("expected schema_version=7 after idempotent re-open, got %q", ver)
	}
}
...
func TestMigrationV3ToV4SelfHealsExistingColumns(t *testing.T) {
	...
	if ver != "7" {
		t.Fatalf("expected schema_version=7 after self-heal, got %q", ver)
	}
	cols := edgeColumns(t, s2)
	if !cols["valid_at"] || !cols["invalid_at"] {
...
func TestMigrationFromV1ReachesV4(t *testing.T) {
	...
	if ver != "7" {
		t.Fatalf("expected schema_version=7 after full ladder, got %q", ver)
	}
	cols := edgeColumns(t, s2)
	if !cols["valid_at"] || !cols["invalid_at"] {
